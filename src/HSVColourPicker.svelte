...
function setStartColor() {
    ...
    if (hex.length !== 6 && hex.length !== 3 && !hex.match(/([^A-F0-9])/gi)) {
        console.error('Invalid property value (startColor)',hex);
        return;
    }
    let hexFiltered='';
    if (hex.length === 3) 
...
function colorChangeCallback(eventType) {
    dispatch(eventType, {
        r: r,
        g: g,
        b: b,
...
function mouseUp(event) {
    if (tracked && tracked.id == elPickerEvent.id) colorChangeCallback('input');
    tracked = null;
}
...
function colorChange() {
    ...
    b = rgb[2];
    hexValue = RGBAToHex();
    if (elColourPicked) elColourPicked.style.background = `rgba(${rgb[0]},${rgb[1]},${rgb[2]},${a})`;
    colorChangeCallback('change');
}
...
function mouseMove(event) {
    ...
    case "colorsquare-event":
        ...
        if (elPicker) {
            elPicker.style.top = yPercentage + "%";
            elPicker.style.left = xPercentage + "%";
        }
        s = xPercentage / 100;
        v = 1 - yPercentage / 100;
        colorChange();
    ...
    case "hue-event":
        ...
        (xPercentage > 100) ? xPercentage = 100: (xPercentage < 0) ? xPercentage = 0 : null;
        xPercentage = xPercentage.toFixed(2);
        if (elHueSelector) elHueSelector.style.left = xPercentage + "%";
        h = xPercentage / 100;
        hueChange();
        break;
    case "alpha-event":
        ...
        (xPercentage > 100) ? xPercentage = 100: (xPercentage < 0) ? xPercentage = 0 : null;
        xPercentage = xPercentage.toFixed(2);
        if (elAlphaSelector) elAlphaSelector.style.left = xPercentage + "%";
        a = xPercentage / 100;
        colorChange();
        break;
    }
...
function hueChange() {
    ...
    elColourSquare.style.background = `rgba(${rgb[0]},${rgb[1]},${rgb[2]},1)`;
    colorChange();
}
...
<div class="main-container">
    ...
    <div class="colorsquare size" bind:this={elColourSquare}>
        <div class="saturation-gradient">
            <div class="value-gradient">
                <div id="colorsquare-picker" bind:this={elPicker}></div>
                <div id="colorsquare-event" bind:this={elPickerEvent} on:mousedown={csDown}></div>
            </div>
        </div>
    </div>

    <div class="hue-selector">
        <div id="hue-picker" bind:this={elHueSelector} ></div>
    ...
    {#if showAlphaSlider}
      <div class="alpha-selector">
          ...
          <div id="alpha-picker" bind:this={elAlphaSelector}></div>
    ...
    {#if showInfoBox}
      <div class="color-info-box">
        <div class="color-picked-bg">
          <div class="color-picked" bind:this={elColourPicked}></div>
        </div>
